...
namespace VSX.UniversalVehicleCombat.Space
{
    ...
    public class SpaceFighterCameraController : VehicleCameraController
    {
        ...
        protected virtual void FieldOfViewUpdate()
        {
            // Update boost field of view effects. 
            if (vehicleEngines != null && vehicleEngines.EnginesActivated == true)
            {
                float targetFieldOfView = vehicleEngines.ModulatedBoostInputs.z * boostFieldOfView +
                                    (1 - vehicleEngines.ModulatedBoostInputs.z) * cameraEntity.DefaultFieldOfView;

                // Set the new field of view
                cameraEntity.SetFieldOfView(Mathf.Lerp(cameraEntity.MainCamera.fieldOfView, targetFieldOfView, boostFieldOfViewLerpSpeed * Time.deltaTime));
