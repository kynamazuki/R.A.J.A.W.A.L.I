...
namespace VSX.UniversalVehicleCombat
{
    ...
    /// </summary>
    public class EnginesVisualEffectsController : MonoBehaviour
    {
        ...
        public List<ExhaustParticle> particles = new List<ExhaustParticle>();
        ...
        public List<ExhaustRenderer> renderers = new List<ExhaustRenderer>();
        ...
        [Tooltip("All the distortion particle systems.")]
        [SerializeField]
        protected List<ParticleSystem> distortionParticleSystems = new List<ParticleSystem>();
        protected ParticleSystem.MainModule[] distortionParticleSystemMainModules;
        protected ParticleSystem.EmissionModule[] distortionParticleSystemEmissionModules;
        protected List<float> distortionParticleStartSpeeds = new List<float>();
        protected List<Material> distortionParticleMaterials = new List<Material>();
        protected List<float> distortionStrengths = new List<float>();
		...
		protected virtual void Awake()
		{
            ...
            // Cache the distortion particle system data

            distortionParticleSystemMainModules = new ParticleSystem.MainModule[distortionParticleSystems.Count];
            distortionParticleSystemEmissionModules = new ParticleSystem.EmissionModule[distortionParticleSystems.Count];
            for (int i = 0; i < distortionParticleSystems.Count; ++i)
            {
                // Cache materials

                ParticleSystemRenderer particleSystemRenderer = distortionParticleSystems[i].GetComponent<ParticleSystemRenderer>();
                foreach (Material mat in particleSystemRenderer.materials)
                {
                    distortionParticleMaterials.Add(mat);
                    distortionStrengths.Add(mat.GetFloat("_DistortionStrengthScaled"));
                }

                // Store the particle speed for dynamic control via throttle (0-1) value

                distortionParticleSystemMainModules[i] = distortionParticleSystems[i].main;
                distortionParticleSystemEmissionModules[i] = distortionParticleSystems[i].emission;
                distortionParticleStartSpeeds.Add(distortionParticleSystemMainModules[i].startSpeed.constant);
            }
        ...
        public virtual void UpdateEffects ()
		{
            ...
            for (int i = 0; i < distortionParticleSystemEmissionModules.Length; ++i)
            {
                distortionParticleSystemEmissionModules[i].enabled = !Mathf.Approximately(effectMultiplier * (cruisingEffectsAmount + boostEffectsAmount), 0);
            }
            ...
            for (int i = 0; i < distortionParticleSystemMainModules.Length; ++i)
            {
                distortionParticleSystemMainModules[i].startSpeed = particleSpeedFactor * distortionParticleStartSpeeds[i];
            }

            // Update distortion strengths
            for (int i = 0; i < distortionParticleMaterials.Count; ++i)
            {
                distortionParticleMaterials[i].SetFloat("_DistortionStrengthScaled", distortionStrengths[i] * Mathf.Clamp(cruisingEffectsAmount + boostEffectsAmount, 0, 1));
            }
        ...
        protected virtual void SetEffectsLevel(float throttleValue, float boostValue)
        {
