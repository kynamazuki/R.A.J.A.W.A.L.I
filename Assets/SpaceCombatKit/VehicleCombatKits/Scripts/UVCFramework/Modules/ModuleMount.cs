...
namespace VSX.UniversalVehicleCombat
{
    ...
    public class ModuleMount : MonoBehaviour
    {
        ...
        protected virtual void Start()
        {
            ...
            if (createDefaultModulesAtStart)
            {
                for (int i = 0; i < defaultModulePrefabs.Count; ++i)
                {
                    ...
                    if (mountableTypes.Count > 0 && !mountableTypes.Contains(defaultModulePrefabs[i].ModuleType))
                    {
        ...
        public virtual bool IsCompatible(Module module)
        {
            ...
            if (mountableTypes.Count > 0 && !mountableTypes.Contains(module.ModuleType))
            {
                return false;
            }
        ...
        public virtual void AddModule(Module module, bool mountImmediately = false)
        {

            if (!IsCompatible(module)) return;
        ...
        public virtual void UnmountActiveModule()
        {
            ...
            {
                int index = mountedModuleIndex;
                modules[index].gameObject.SetActive(false);
                modules[index].OnUnmounted();
                mountedModuleIndex = -1;
                onModuleUnmounted.Invoke(modules[index]);
