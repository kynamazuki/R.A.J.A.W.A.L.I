...
namespace VSX.UniversalVehicleCombat.Radar
{
    ...
    public class HUDTargetBoxes : HUDComponent
    {
        ...
        [Tooltip("Whether to set the target box layer (including its children) to the layer of this gameobject.")]
        [SerializeField]
        protected bool setTargetBoxToLayer = false;
        ...
        [SerializeField]
        protected bool rotateOffScreenUI = true;

        [SerializeField]
        protected float offscreenArrowsViewportSpaceRadius = 0.2f;

        [SerializeField]
        protected float offscreenArrowsViewportMinX = 0;

        [SerializeField]
        protected float offscreenArrowsViewportMaxX = 1;

        [SerializeField]
        protected float offscreenArrowsViewportMinY = 0;

        [SerializeField]
        protected float offscreenArrowsViewportMaxY = 1;

        [Tooltip("Whether to display a target as selected when the target selector references one of its subsystems.")]
        [SerializeField]
        protected bool displaySelectedTargetParentAsSelected = true;
        ...
        protected Vector2 GetWorldSpaceSize(Vector2 viewportSize, float distanceFromCamera)
        {
            ...
            float opposite = Mathf.Tan(m_HUDCamera.fieldOfView * 0.5f * Mathf.Deg2Rad) * distanceFromCamera;
            float worldSpaceScreenHeight = opposite * 2;
            float worldSpaceScreenWidth = m_HUDCamera.aspect * worldSpaceScreenHeight;
        ...
        protected void AlignHUDElement(RectTransform hudElement, Vector3 centeredViewportPosition, Quaternion addedRotation, float zDistance = 0, bool alignRelative = false, Vector3 relativeReference = new Vector3())
        {
            ...
            if (!isWorldSpace)
            {
                ...
                hudElement.localRotation = Quaternion.identity;
            ...
            {
                hudElement.position = m_HUDCamera.ViewportToWorldPoint(new Vector3(centeredViewportPosition.x + 0.5f, centeredViewportPosition.y + 0.5f, zDistance));
                hudElement.rotation = Quaternion.LookRotation(hudElement.position - m_HUDCamera.transform.position, m_HUDCamera.transform.up);
                hudElement.rotation *= addedRotation;
            }
        }
        ...
        protected virtual void ShowTargetBox(Tracker tracker, Trackable trackable, HUDTargetBox targetBox)
        {

            // Exit if no target box found
            if (targetBox == null) return;

            targetBox.PreTargetBoxUpdate();

            if (setTargetBoxToLayer) targetBox.SetLayer(gameObject.layer);
            ...
            {
                if (tracker.TargetSelectors[i].SelectedTarget != null)
                {
                    if (tracker.TargetSelectors[i].SelectedTarget == trackable || (displaySelectedTargetParentAsSelected && tracker.TargetSelectors[i].SelectedTarget.ParentTrackable == trackable))
                    {
            ...
            if (isWorldSpace)
            {
                ...
                {
                    distanceFromCamera = Vector3.Distance(m_HUDCamera.transform.position, trackable.transform.position);
                }
            ...
            Vector3 centeredViewportPos = HUDTargetBoxesFunctions.GetViewportPosition(position, m_HUDCamera, true);
            ...
            if (!isInView)
            {
                switch (targetBoxOffScreenMode)
                {
                    ...
                    case TargetBoxOffScreenMode.CenterRadial:
                        ...
                        centeredViewportPos.x = clampedCenteredViewportPosition.x * (1 / m_HUDCamera.aspect);
                        centeredViewportPos.y = clampedCenteredViewportPosition.y;

                        centeredViewportPos.x = Mathf.Clamp(centeredViewportPos.x, offscreenArrowsViewportMinX - 0.5f, offscreenArrowsViewportMaxX - 0.5f);
                        centeredViewportPos.y = Mathf.Clamp(centeredViewportPos.y, offscreenArrowsViewportMinY - 0.5f, offscreenArrowsViewportMaxY - 0.5f);
            ...
            AlignHUDElement(targetBox.RectTransform, centeredViewportPos, Quaternion.Euler(0f, 0f, rotateOffScreenUI ? targetBoxAngle : 0), distanceFromCamera);

            // Update the size of the target box
            if (canvas.renderMode == RenderMode.WorldSpace)
            {
                Vector2 viewportSize = HUDTargetBoxesFunctions.GetViewportSize(trackable, extentsCornersArray, m_HUDCamera, viewportRect);
                targetBox.SetSize(GetWorldSpaceSize(viewportSize, distanceFromCamera));
            }
            else
            {
                targetBox.SetSize(Vector2.Scale(HUDTargetBoxesFunctions.GetViewportSize(trackable, extentsCornersArray, m_HUDCamera, canvasRect), canvasRectTransform.sizeDelta));
            }
            ...
            if (leadTargetInfo != null)
            {
                if (leadTargetInfo.Target == trackable.transform)
                {
                    for (int i = 0; i < leadTargetInfo.LeadTargetPositions.Length; ++i)
                    {
                        ...
                        {
                            Vector3 viewportPos = HUDTargetBoxesFunctions.GetViewportPosition(leadTargetInfo.LeadTargetPositions[i], m_HUDCamera, true);
                            ...
                            leadTargetBoxController.UpdateLeadTargetBox(leadTargetInfo.LeadTargetAimedIndex == i, leadTargetInfo.AimStateStartTime);
                        }
                    }
                }
            }
