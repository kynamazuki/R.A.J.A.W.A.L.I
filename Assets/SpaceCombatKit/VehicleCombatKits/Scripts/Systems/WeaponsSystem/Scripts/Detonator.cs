...
namespace VSX.UniversalVehicleCombat
{
    ...
    public class Detonator : MonoBehaviour
    {
        ...
        {
            Detonate(transform.position, transform.forward, detonatingDuration);
        }
        ...
        {
            Detonate(transform.position, transform.forward, 0);
        }
        ...
        public virtual void Detonate(RaycastHit hit)
        {
            ...
            Detonate(hit.point, hit.normal, detonatingDuration, true);
        }
        ...
        public virtual void Detonate(Vector3 detonationPosition, Vector3 detonationForward, float thisDetonatingDuration = 0, bool specifiedDetonationPosition = false)
        {
            ...
            // Start the coroutine
            if (gameObject.activeInHierarchy) StartCoroutine(DetonationCoroutine(thisDetonatingDuration));
        ...
        protected virtual IEnumerator DetonationCoroutine(float thisDetonatingDuration)
        {
            SetDetonationState(DetonationState.Detonating);
            if (thisDetonatingDuration > 0) yield return new WaitForSeconds(thisDetonatingDuration);
