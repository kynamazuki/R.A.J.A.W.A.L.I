    ...
    public class LoadoutVehicleSpawner : MonoBehaviour
    {
        ...
        IEnumerator SpawnCoroutine()
        {
            ...
            if (data != null)
            {
                ...
                if (loadSelectedSlot)
                {
                    thisSlotIndex = data.Slots.IndexOf(data.SelectedSlot);
                    vehicleIndex = data.SelectedSlot.selectedVehicleIndex;
                }
                else
                {
                    if (data.Slots.Count > slotIndex)
                    {
                        thisSlotIndex = slotIndex;
                        vehicleIndex = data.Slots[slotIndex].selectedVehicleIndex;
                    }
                }
                
                if (vehicleIndex != -1)
                {
                    spawnedVehicle = Instantiate(loadoutItems.vehicles[vehicleIndex].vehiclePrefab, transform.position, transform.rotation);
                    ...
                    for (int i = 0; i < data.Slots[thisSlotIndex].selectedModules.Count; ++i)
                    {
                        int moduleIndex = data.Slots[thisSlotIndex].selectedModules[i];
                        if (moduleIndex != -1)
                        {
                            Module module = Instantiate(loadoutItems.modules[moduleIndex].modulePrefab, transform.position, transform.rotation);
