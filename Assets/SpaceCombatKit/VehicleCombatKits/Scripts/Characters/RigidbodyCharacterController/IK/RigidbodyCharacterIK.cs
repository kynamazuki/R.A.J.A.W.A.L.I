...
public class RigidbodyCharacterIK : MonoBehaviour
{
    ...
    protected float characterHandleHeightOffset;
    ...
    protected void Reset()
    {
        ...
        {
            characterHandle = animatorController.transform.parent;
        }
    ...
    {
        characterHandleHeightOffset = characterHandle.transform.position.y - characterController.transform.position.y;
    }
    ...
    protected virtual void UpdateCharacterPosition()
    {
        ...
        if (characterController.Grounded)
        {
            for (int i = 0; i < legs.Count; ++i)
            {
                ...
                // Do raycast
                RaycastHit[] hits = Physics.RaycastAll(raycastPos, -Vector3.up, legs[i].GetLength() * 2, characterController.GroundMask, QueryTriggerInteraction.Ignore);
                if (characterController.GetGround(hits, out hit))
                {
        ...
        {
            characterHandle.localPosition = new Vector3(0, -drop + characterHandleHeightOffset, 0);
        }
    }
    ...
    protected virtual void UpdateCharacterPlacement()
    {
        ...
        for (int i = 0; i < legs.Count; ++i)
        {
            ...
            if (GetFootSurfaceTarget(legs[i], out hitPoint, out hitNormal))
            {
                ...
                float heightAmount = Mathf.Max((legs[i].foot.m_Transform.position.y - legs[i].footHeight) - characterHandle.position.y + characterHandleHeightOffset, 0) /
    ...
    protected virtual bool GetFootSurfaceTarget(IKLeg leg, out Vector3 position, out Vector3 normal)
    {
        ...
        // Perform the raycast
        RaycastHit hit;
        RaycastHit[] hits = Physics.RaycastAll(raycastPos, -Vector3.up, leg.GetLength() * 2, characterController.GroundMask, QueryTriggerInteraction.Ignore);
        if (characterController.GetGround(hits, out hit))
        {
            position = hit.point;
            normal = hit.normal;
