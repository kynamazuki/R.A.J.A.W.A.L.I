...
namespace VSX.UniversalVehicleCombat
{
    ...
    public class RigidbodyCharacterController : MonoBehaviour
    {
        ...
        public virtual bool GetGround(RaycastHit[] hits, out RaycastHit groundHit)
        {
            bool found = false;
            ...
            {
                if (hits[i].collider.attachedRigidbody != null && hits[i].collider.attachedRigidbody == m_Rigidbody) continue;
                if (hits[i].collider.transform.root == transform) continue;
        ...
        protected virtual void CheckIsGrounded()
        {
            ...
            // Do a spherecast
            RaycastHit[] hits = Physics.SphereCastAll(startingPos, capsuleCollider.radius, -Vector3.up, castDistance, groundMask, QueryTriggerInteraction.Ignore);
            if (GetGround(hits, out hit))
            {
                ...
                RaycastHit[] hits2 = Physics.RaycastAll(startingPos, -Vector3.up, castDistance + capsuleCollider.radius, groundMask, QueryTriggerInteraction.Ignore);
                if (!GetGround(hits2, out raycastGroundHit))
                {
                    if (grounded)
                    {
